...
\-D<b>variant\_FEATURE\_MAX\_ALIGN\_HACK</b>=0  
Define this to 1 to use the max align hack for alignment. Default is 0.

\-D<b>variant\_FEATURE\_ALIGN\_AS</b>=*pod_type*  
Define this to the pod-type you want to align to (no default).

\-D<b>variant\_FEATURE\_ALIGN\_AS\_FALLBACK</b>=*pod_type*  
...
2. If you define -D<b>variant_FEATURE_MAX_ALIGN_HACK</b>=1 the underlying type is aligned as the most restricted type in `struct max_align_t`. This potentially wastes many bytes per variant if the actually required alignment is much less, e.g. 24 bytes used instead of the 2 bytes required.

3. If you define -D<b>variant_FEATURE_ALIGN_AS</b>=*pod-type* the underlying type is aligned as *pod-type*. It's your obligation to specify a type with proper alignment.

4. If you define -D<b>variant_FEATURE_ALIGN_AS_FALLBACK</b>=*pod-type* the fallback type for alignment of rule 5 below becomes *pod-type*. It's your obligation to specify a type with proper alignment.
...
5. At default, *variant lite* tries to find a POD type with the same alignment as the underlying type.
	...
	- If no such POD type is found, use a type with a relatively strict alignment requirement such as double; this type is specified in  `variant_FEATURE_ALIGN_AS_FALLBACK` (default double).
